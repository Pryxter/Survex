...
export default function CpxPage() {
  ...
  const [source, setSource] = useState<"cpx" | "demo" | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`${apiBaseUrl}/api/cpx/surveys`, {
      headers: {
      ...
      <div className="mx-auto w-full max-w-7xl p-6 md:p-8">
        ...
        <section className="mt-8 rounded-3xl border border-white/10 bg-slate-900/80 p-6 md:p-8">
          ...
          <p className="mt-3 max-w-2xl text-slate-300">
            Complete CPX Research surveys and receive rewards. Rewards are
            validated server-side before balance updates are applied.
          </p>
          ...
          {source === "demo" && !errorMessage ? (
            <p className="mt-4 rounded-xl border border-amber-300/30 bg-amber-400/10 px-4 py-3 text-sm text-amber-200">
              Demo mode active. Configure CPX Research credentials in backend
