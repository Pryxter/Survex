...
type SurveysResponse = {
  source: "inbrain" | "demo";
  surveys: InBrainSurvey[];
  message?: string;
};

export default function InBrainPage() {
  const router = useRouter();
  const [loading, setLoading] = useState(true);
  const [source, setSource] = useState<"inbrain" | "demo" | null>(null);
  const [surveys, setSurveys] = useState<InBrainSurvey[]>([]);
  ...
  useEffect(() => {
    ...
    fetch(`${apiBaseUrl}/api/inbrain/surveys`, {
      headers: {
        ...
        <section className="mt-8 rounded-3xl border border-white/10 bg-slate-900/80 p-6 md:p-8">
          ...
          {source === "demo" && !errorMessage ? (
            <p className="mt-4 rounded-xl border border-amber-300/30 bg-amber-400/10 px-4 py-3 text-sm text-amber-200">
              Demo mode active. Configure InBrain.ai URL template in backend env
