...
type SurveysResponse = {
  source: "tapresearch" | "demo";
  surveys: TapResearchSurvey[];
  message?: string;
};

export default function TapResearchPage() {
  const router = useRouter();
  const [loading, setLoading] = useState(true);
  const [source, setSource] = useState<"tapresearch" | "demo" | null>(null);
  const [surveys, setSurveys] = useState<TapResearchSurvey[]>([]);
  ...
  useEffect(() => {
    ...
    fetch(`${apiBaseUrl}/api/tapresearch/surveys`, {
      headers: {
      ...
      <div className="mx-auto w-full max-w-7xl p-6 md:p-8">
        ...
        <section className="mt-8 rounded-3xl border border-white/10 bg-slate-900/80 p-6 md:p-8">
          ...
          <p className="mt-3 max-w-2xl text-slate-300">
            Live campaigns loaded via TapResearch API. Rewards and reversals are
            validated server-side to protect your balance.
          </p>
          ...
          {source === "demo" && !errorMessage ? (
            <p className="mt-4 rounded-xl border border-amber-300/30 bg-amber-400/10 px-4 py-3 text-sm text-amber-200">
              Demo mode active. Configure TapResearch API credentials in backend env
